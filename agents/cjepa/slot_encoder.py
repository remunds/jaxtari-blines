...
class SlotEncoder(nn.Module):
    ...
    Input:  flattened OC observation [B, T, stacked_obj_attr_dim * num_objects]
            where stacked_obj_attr_dim = obj_attr_dim * frame_stack_size.
    Reshapes to [B, T, S, stacked_obj_attr_dim] where S = num_slots (= num_objects).
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        # x: [B, T, stacked_obj_attr_dim * num_slots]
        B, T, _ = x.shape
        stacked_dim = self.obj_attr_dim * self.frame_stack_size

        # Reshape to [B, T, S, stacked_obj_attr_dim]
        x = x.reshape(B, T, self.num_slots, stacked_dim)
        ...
        # Input: [B, T, S, stacked_obj_attr_dim] → vmap over S → [B, T, S, slot_dim]
